    ...
    <header className="bg-uno sticky top-0 z-40 w-full text-Textos">
        <div className=" p-2 container w-full mx-auto flex justify-between items-center">
            ...
            <nav className=" hidden  md:flex bg-fondo p-2 w-[75%] items-center justify-center space-x-6">
                {menuItems.map((item) => (
                    <Link key={item.name} href={item.href} className=" hover:text-dos font-semibold text-texto text-lg transition-all duration-300 ease-in-out relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-Botones after:transition-all after:duration-300 hover:after:w-full">
                        {item.name}
                    </Link>
                ))}
            </nav>
