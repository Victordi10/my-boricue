...
// Estilos predefinidos que pueden seleccionarse mediante una prop
const presetStyles = {
    default: {
        bgColor: 'bg-uno',
...
const Header = ({
    ...
    // Obtener el preset de estilo seleccionado o usar el predeterminado
    ...
    return (
        <header className={`sticky top-0 z-40 w-full transition-all duration-300 ${bgColor} ${textColor} ${scrolled ? 'shadow-md py-1' : 'py-2'} ${className}`}>
            <div className="container mx-auto px-4 flex justify-between items-center">
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`absolute bottom-0 left-0 w-0 h-0.5 ${accentColor} transition-all duration-300 ease-in-out hover:w-full`} />
                            </Link>
                        )
                    })}
                </nav>
                ...
                <button
                    className={`md:hidden w-10 h-10 flex items-center justify-center rounded-full ${isOpen ? 'bg-opacity-20' : ''} focus:outline-none transition-colors duration-300`}
                    onClick={() => setIsOpen(!isOpen)}
                    aria-label={isOpen ? "Cerrar menú" : "Abrir menú"}
                >
                    {isOpen ? (
                        <X size={24} className={hoverColor} />
                    ) : (
